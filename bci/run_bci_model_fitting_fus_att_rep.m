...
% model
model='fus'; % bci fus segA segV

% initialise cell for best 10 results
group_best10_results_mod_switch = cell(length(subjID),1);

% initialise structure for best results
group_best_results_mod_switch = cell(length(subjID),1);
...
for iSubj = 1:length(subjID)
    ...
    if ~sum(strcmp(origData.Properties.VarNames(:),'AttentionModalityBCI')) % if AttentionModality is not present
        ...
        for j = 1:size(origData,1)
            % A no switch = 1
            if (strcmp(origData.AttentionModality(j), 'Aud') && strcmp(origData.ResponseModality(j), 'Aud'))
                origData.AttentionModalityBCI(j) = 1;
            % A switch = 2
            elseif (strcmp(origData.AttentionModality(j), 'Aud') && strcmp(origData.ResponseModality(j), 'Vis'))
                origData.AttentionModalityBCI(j) = 2;
            % V no switch = 3    
            elseif (strcmp(origData.AttentionModality(j), 'Vis') && strcmp(origData.ResponseModality(j), 'Aud'))
                origData.AttentionModalityBCI(j) = 3;
            % V switch = 4    
            elseif (strcmp(origData.AttentionModality(j), 'Vis') && strcmp(origData.ResponseModality(j), 'Vis'))
                origData.AttentionModalityBCI(j) = 4;
            end            
        end
    end
    ...
    sigV3 = linspace(0.1,30,n);
    sigV4 = linspace(0.1,30,n);
    sigA1 = linspace(0.1,30,n);
    sigA2 = linspace(0.1,30,n);
    sigA3 = linspace(0.1,30,n);
    sigA4 = linspace(0.1,30,n);
    ...
    % Parameter names
    if strcmp(model, 'fus')
        parameterNames = {'sigP','sigA1','sigA2','sigA3','sigA4','sigV1','sigV2','sigV3','sigV4'};
        gridVectors = {sigP,sigA1,sigA2,sigA3,sigA4,sigV1,sigV2,sigV3,sigV4};
    elseif strcmp(model, 'segA')
        parameterNames = {'sigP','sigA1','sigA2','sigA3','sigA4'};
        gridVectors = {sigP,sigA1,sigA2,sigA3,sigA4};
    elseif strcmp(model, 'segV')
        parameterNames = {'sigP','sigV1','sigV2','sigV3','sigV4'};
        gridVectors = {sigP,sigV1,sigV2,sigV3,sigV4};
    else
        parameterNames = {'p_common','sigP','sigA1','sigA2','sigA3','sigA4','sigV1','sigV2','sigV3','sigV4'};
        gridVectors = {p_common,sigP,sigA1,sigA2,sigA3,sigA4,sigV1,sigV2,sigV3,sigV4};
    end
    ...
    fprintf('\n\nSaving data...\n\n');
    save([subjID{iSubj} '_' model 'Simulations_attmod_switch_gridsearch'], ...
    ...
    if figure_plot==1
        ...
        saveas(gcf,[subjID{iSubj} '_gridsearch_attmod_switch_logLike' ],'emf');
    end
    ...
    % 'p_common', 'sigP', 'sigA1', 'sigA2', 'sigA3', 'sigA4', 'sigV1', 'sigV2', 'sigV3', 'sigV4'
    if strcmp(model, 'fus')
        LB = [0.001 0.001 0.001 0.001 0.001 0.001 0.001 0.001 0.001];
        UB = [30    30    30    30    30    30    30    30    30];
    elseif strcmp(model, 'segA') || strcmp(model, 'segV')
        LB = [0.001 0.001 0.001 0.001 0.001];
        UB = [30    30    30    30    30];
    else
        LB = [0 0.001 0.001 0.001 0.001 0.001 0.001 0.001 0.001 0.001];
        UB = [1 30    30    30    30    30    30    30    30    30   ];
    end
    ...
    group_best10_results_mod_switch{iSubj} = subj_results;
    ...
    group_best_results_mod_switch{iSubj}.fmin_bestlogLike = fmin_bestlogLike;
    group_best_results_mod_switch{iSubj}.bci_details = bci_details;
    group_best_results_mod_switch{iSubj}.bic = bic;
    group_best_results_mod_switch{iSubj}.aic = aic;
