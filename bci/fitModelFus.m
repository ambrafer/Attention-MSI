function [negLogLike,mdlEval] = fitModelFus(parameters,parameterNames,dataVA,responseLoc)
...
% USAGE:
%   [logLike,all] = bci.fitModelFus(parameters,parameterNames,dataVA,responseLoc)
% INPUT:
...
%       Possible values: 'muP','sigP','kW','sigA','sigV'
...
if any(cellfun(@isempty,{muP,sigP,sigA,sigV,kernelWidth}))
...
for indCond = 1:nCond
    ...
    s_hat_common = (xV/varV + xA/varA + muP/varP) * varVA_hat;
    ...
    if length(responseLoc) > 1
        ...
        [~,tV] = min(abs(repmat(s_hat_common,1,lengthRespLoc) - repmat(responseLoc,nIntSamples,1)),[],2);
        [~,tA] = min(abs(repmat(s_hat_common,1,lengthRespLoc) - repmat(responseLoc,nIntSamples,1)),[],2);
        sV_pred_resp = responseLoc(tV);
        sA_pred_resp = responseLoc(tA);
    else
        %continuous responses used, no discretisation
        sV_pred_resp = s_hat_common;
        sA_pred_resp = s_hat_common;
    end
    ...
    if length(responseLoc) > 1
        ...
        %calculate log-likelihood Kording 2007 Eq. 16, page 8
