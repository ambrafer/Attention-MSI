...
% model
model='bci'; % bci fus segA segV taskRel

% initialise cell for best 10 results
group_best10_results_mod = cell(length(subjID),1);

% initialise structure for best results
group_best_results_mod = cell(length(subjID),1);
...
for iSubj = 1:length(subjID)
    ...
    if ~sum(strcmp(origData.Properties.VarNames(:),'AttentionModalityBCI')) % if AttentionModality is not present
        ...
        for j = 1:size(origData,1)
            % Attention Auditory = 1
            if (strcmp(origData.AttentionValidity(j), 'Attended') && strcmp(origData.ResponseModality(j), 'Aud')) || ...
                    (strcmp(origData.AttentionValidity(j), 'Unattended') && strcmp(origData.ResponseModality(j), 'Vis'))
                origData.AttentionModalityBCI(j) = 1;
                % Attention Visual = 2
            elseif (strcmp(origData.AttentionValidity(j), 'Attended') && strcmp(origData.ResponseModality(j), 'Vis')) || ...
                    (strcmp(origData.AttentionValidity(j), 'Unattended') && strcmp(origData.ResponseModality(j), 'Aud'))
    ...
    fprintf('\n\nSaving data...\n\n');
    save([subjID{iSubj} '_' model 'Simulations_attmod_gridsearch'], ...
    ...
    if figure_plot==1
        ...
        saveas(gcf,[subjID{iSubj} '_gridsearch_attmod_logLike' ],'emf');
    end
    ...
    group_best10_results_mod{iSubj} = subj_results;
    ...
    group_best_results_mod{iSubj}.fmin_bestlogLike = fmin_bestlogLike;
    group_best_results_mod{iSubj}.bci_details = bci_details;
    group_best_results_mod{iSubj}.bic = bic;
    group_best_results_mod{iSubj}.aic = aic;
